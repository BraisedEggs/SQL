...
![三个表.png](assest/%E4%B8%89%E4%B8%AA%E8%A1%A8.png)

### (普通查询)查询员工名为'Abel'的人在哪个城市工作？
...
#### 错误查询(每个员工都与每个部门匹配了一遍)

* SELECT employee_id,department_name FROM employees,departments(2889条记录)
* 原因(查出来的数据是每个员工都在每个部门)
    * SELECT *
...
### 如果给表起了别名，一旦在SELECT或WHERE中使用表名的话，则必须使用表的别名，而不能再使用表的原名。(别名会覆盖本身名称)
...
### 7.1、角度1：等值连接 vs 非等值连接

* 前面都是等值连接

* 非等值连接
    * 查询每个人的工资等级
        * SELECT emp.last_name,emp.salary,jobs.grade_level
          FROM employees emp,job_grades jobs
          WHERE emp.salary BETWEEN jobs.lowest_sal AND jobs.highest_sal
...
* 自连接
  ![自我链接.png](assest/%E8%87%AA%E6%88%91%E9%93%BE%E6%8E%A5.png)
...
#### *外连接：合并具有同一列的两个以上的表的行, 结果集中除了包含一个表与另一个表匹配的行之外，还查询到了左表 或
...
* 外连接的分类：左外连接、右外连接、满外连接
    ...
    * ![外连接.png](assest/%E5%A4%96%E8%BF%9E%E6%8E%A5.png)

##### 练习

* 练习：查询所有的员工的last_name,department_name信息 (所有可能查不匹配的 外连接)
    * SELECT employee_id,department_name
      FROM employees e,departments d
...
* UNION
    ...
    * ![union.png](assest/union.png)
* UNION ALL(开发用的多)
    * UNION ALL操作符返回两个查询的结果集的并集。对于两个结果集的重复部分，不去重。
    * ![union_all.png](assest/union_all.png)

## 9、7种SQL JOINS的实现

![7中jionsOn.png](assest/7%E4%B8%ADjionsOn.png)
